...
let aiState = { model: null, videoStream: null, cameraActive: false };
...
function renderSequence() {
    ...
    document.getElementById('sequenceSlots').innerHTML = seq.steps.map((_, i) => `
        <div class="sequence-slot" id="seq-slot-${i}">
...
function renderStories() {
    ...
    document.getElementById('storiesContainer').innerHTML = `
        <div class="story-card">
            ...
            <p>${story.text}</p>
            <button class="btn btn-play btn-sm" onclick="readStory()"><i class="fas fa-volume-up"></i> اقرأ لي القصة</button>
        </div>
    `;
}
function readStory() {
...
function generateRecommendations() {
    ...
    if (successRate < 30) {
        recommendations.push({
            icon: '🔢', title: 'ابدأ بالتسلسلات', desc: 'التسلسلات أسهل وتحسن ثقة الطفل.',
            action: () => { switchTab('sequences', document.getElementById('tab-sequences')); }
        }, {
            icon: '😊', title: 'تعرف على المشاعر', desc: 'نشاط بصري بسيط يبني الثقة.',
            action: () => { switchTab('emotions', document.getElementById('tab-emotions')); }
        });
    } else if (successRate < 60) {
        recommendations.push({
            icon: '🎤', title: 'استمر في الكلمات', desc: 'أداء جيد! استمر في الكلمات السهلة.',
            action: () => { switchTab('words', document.getElementById('tab-words')); }
        });
    } else {
        recommendations.push({
            icon: '💬', title: 'المحادثات الموجهة', desc: 'أداء ممتاز! جرب المحادثات المتقدمة.',
    ...
    content.innerHTML = `
        <div style="margin-bottom:15px;">
            <div style="display:flex;justify-content:space-between;align-items:center;">
                ...
                <span style="color:#34d399;font-weight:900;font-size:1.5rem;">${successRate}%</span>
            </div>
        </div>
        <div style="display:grid;gap:10px;">
            ${recommendations.map((rec, i) => `
                <div onclick="window.recommendations[${i}].action()" style="padding:15px;background:rgba(255,255,255,0.05);border-radius:12px;cursor:pointer;">
                    <div style="display:flex;align-items:center;gap:12px;">
                        <span style="font-size:2rem;">${rec.icon}</span>
                        <div>
                            <div style="font-weight:800;color:white;">${rec.title}</div>
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        document.getElementById('sessionEndModal').classList.add('hidden');
        if(document.getElementById('recommendationsPanel')) document.getElementById('recommendationsPanel').classList.add('hidden');
    }
});

console.log('Autism AI App loaded successfully');
ewWordText').value = ''; document.getElementById('newWordEmoji').value = '';
    showToast('✅ تمت الإضافة', 'success');
}
function deleteWord(id) {
    data.words = data.words.filter(w => w.id !== id); saveData(); renderAdminLists(); renderWords();
