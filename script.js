...
function showRecommendations() {
    ...
    if (!panel.classList.contains('hidden')) {
        ...
        if (typeof gtag !== 'undefined') gtag('event', 'recommendations_open');
    }
}
function generateRecommendations() {
    ...
    if (successRate < 30) {
        recommendations.push({
            icon: '🔢', title: 'ابدأ بالتسلسلات', desc: 'نسبة النجاح منخفضة. التسلسلات أسهل وتحسن الثقة.',
            action: () => { switchTab('sequences', document.getElementById('tab-sequences')); }
        });
        recommendations.push({
            icon: '😊', title: 'تعرف على المشاعر', desc: 'نشاط بصري بسيط يبني الثقة قبل الكلمات.',
            action: () => { switchTab('emotions', document.getElementById('tab-emotions')); }
        });
    } else if (successRate < 60) {
        recommendations.push({
            icon: '🎤', title: 'استمر في الكلمات', desc: 'أداء جيد! استمر في الكلمات السهلة والمتوسطة.',
            action: () => { switchTab('words', document.getElementById('tab-words')); }
        });
        recommendations.push({
            icon: '👋', title: 'التعليمات الحركية', desc: 'أضف تنوعاً بالتعليمات الحركية لتعزيز الاستجابة.',
        ...
        recommendations.push({
            icon: '📂', title: 'التصنيفات', desc: 'تحدي جديد: تصنيف الأشياء إلى مجموعات.',
    ...
    if (weakWords.length > 0) {
        recommendations.push({
            icon: '📝', title: 'كلمات تحتاج تدريب', desc: `ركز على: ${[...new Set(weakWords)].slice(0, 3).join('، ')}`,
            action: () => { switchTab('words', document.getElementById('tab-words')); }
        });
    }
    content.innerHTML = `
...
document.addEventListener('DOMContentLoaded', function() {
    // Ensure all screens are properly hidden on load
    document.getElementById('appScreen').classList.add('hidden');
    document.getElementById('mainScreen').classList.add('hidden');
    document.getElementById('parentDashboard').classList.add('hidden');
    document.getElementById('adminDashboard').classList.add('hidden');
...
style.textContent = `@keyframes float { 0%, 100% { transform: translateY(0px); } 50% { transform: translateY(-15px); } }`; 
